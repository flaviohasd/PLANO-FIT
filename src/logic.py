...
def analisar_historico_treinos(dft: pd.DataFrame) -> Dict[str, Any]:
    ...
    Returns:
        Dict[str, Any]: Dicionário com métricas como total de treinos,
                        média por semana, etc. Retorna vazio se o input for vazio.
    """
    if dft.empty: return {}
...
def get_latest_metrics(dados_pessoais: Dict[str, Any], df_evolucao: pd.DataFrame) -> Dict[str, Any]:
    ...
    - Se houver valores válidos (>0) no histórico, utiliza o último (mais recente) diferente de zero.
    - Caso não encontre nenhum valor válido no histórico, mantém o valor presente em dados_pessoais.
    Observações:
    - Trata vírgulas decimais, espaços em colunas e possíveis variações de nome de coluna (aliases).
    - Não altera dados_pessoais; apenas retorna um dicionário com as métricas a serem usadas na UI.
    """
    latest_metrics = dados_pessoais.copy()

    # Se não há histórico, retorna os dados pessoais tal como estão
    if df_evolucao is None or df_evolucao.empty:
        return latest_metrics

    # Trabalhar em cópia para não alterar o dataframe original
    df_sorted = df_evolucao.copy()

    # Normaliza nomes de colunas (remove espaços indesejados)
    df_sorted.columns = df_sorted.columns.astype(str).str.strip()

    # Determina a coluna de data (usa config.COL_DATA se existir, caso contrário tenta 'data')
    col_data = getattr(config, "COL_DATA", "data")
    if col_data not in df_sorted.columns and "data" in df_sorted.columns:
        col_data = "data"

    # Converte a coluna de data para datetime (tenta com o formato esperado)
    ...
    df_sorted = df_sorted.sort_values(by='data_dt', ascending=False).reset_index(drop=True)

    # Mapeamento das métricas que queremos popular no resultado -> possíveis nomes/aliases no histórico
    # Chave: nome final usado em latest_metrics; valor: lista de colunas possíveis em df_evolucao
    metric_aliases = {
    ...
    # Função auxiliar: converte série para numérico de forma robusta
    ...
    # Para cada métrica, procura o primeiro valor válido (>0) do histórico (mais recente primeiro)
    for metric_key, aliases in metric_aliases.items():
        found_value = None

        # procurar a primeira coluna disponível entre os aliases
        col_found = None
        ...
        if col_found is None:
            # Coluna não existe no histórico; continua (mantém valor em dados_pessoais)
            continue

        # Converte a coluna encontrada para numérico
        series_num = _to_numeric_series(df_sorted[col_found])

        # Filtra valores não-nulos e diferentes de zero (considerando também > 0)
        valid_mask = series_num.notna() & (series_num != 0)
        if valid_mask.any():
            # Como o df está ordenado da data mais recente para a mais antiga, o primeiro elemento válido é o desejado
            first_valid_idx = valid_mask.idxmax()  # primeiro True
            val = series_num.loc[first_valid_idx]
            # Garante float simples no resultado
            try:
                found_value = float(val)
            except Exception:
                found_value = None

        # Se encontrou um valor válido, atualiza latest_metrics[metric_key]
        if found_value is not None:
            latest_metrics[metric_key] = found_value
        # Caso contrário, mantém o valor já presente em dados_pessoais (não sobrescreve com zero)
