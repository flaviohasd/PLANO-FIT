...
def get_workout_for_day(user_data: Dict[str, Any], target_date: date) -> Dict[str, Any] or None:
    ...
    Args:
        user_data (Dict[str, Any]): O dicionário com todos os DataFrames do usuário.
        target_date (date): A data para a qual o treino será buscado.

    Returns:
        Dict[str, Any] or None: Um dicionário com o nome do plano e um DataFrame
                                de exercícios, ou None se nenhum treino for encontrado.
    """
    ...
    exercicios_do_plano = df_exercicios[df_exercicios['id_plano'] == id_plano] if 'id_plano' in df_exercicios.columns else pd.DataFrame()
