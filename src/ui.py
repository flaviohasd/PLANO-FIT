...
def render_alimentacao_tab(user_data: Dict[str, Any], TABELA_ALIM: pd.DataFrame, RECOMEND: pd.DataFrame):
    ...
    with col_refeicoes:
        ...
        # --- MODIFICAÇÃO: Usa a função cacheada para obter os dados processados ---
        ...
        # Comparação com as metas diárias (lógica de renderização rápida)
        df_obj = user_data.get("df_objetivo", pd.DataFrame())
        ...
        # Só exibe os gráficos se houver dados (obtidos do cache)
        ...
        # Ordena as refeições por nome antes de exibir
        sequencia_refeicao = ["Cafe da manha", "Lanche da manha", "Almoco", "Lanche da tarde", "Jantar", "Pos-treino", "Lanche da noite"]

        # Supondo que df_refeicoes já tenha sido criado anteriormente
        # Verificando se o DataFrame não está vazio e contém a coluna 'Refeicao'
        if not df_refeicoes.empty and "Refeicao" in df_refeicoes.columns:
            # Definir a coluna 'Refeicao' como uma categoria ordenada com a sequência desejada
            df_refeicoes['Refeicao'] = pd.Categorical(df_refeicoes['Refeicao'], categories=sequencia_refeicao, ordered=True)
            
            # Ordenar o DataFrame com base na coluna 'Refeicao'
            df_refeicoes = df_refeicoes.sort_values(by="Refeicao", ascending=True).reset_index(drop=True)

        # Configuração do editor de dados no Streamlit
        ...
        if st.button("💾 Salvar Alterações Manuais", key="salvar_refeições"):
            ...
            # --- MODIFICAÇÃO: Limpa o cache após salvar ---
    ...
    with st.expander("Gerenciar Meus Planos Alimentares", expanded=True):
        ...
        if plano_selecionado == "-- Criar Novo Plano --":
            ...
            st.button(
                ...
                args=(novo_nome_plano, lista_planos)
            )
            ...
            # Sequência personalizada para as refeições
            sequencia_refeicao = ["Cafe da manha", "Lanche da manha", "Almoco", "Lanche da tarde", "Jantar", "Pos-treino", "Lanche da noite"]

            # Filtrando os dados para o plano selecionado
            ...
            if not itens_plano.empty and "Refeicao" in itens_plano.columns:
                # Definir a coluna 'Refeicao' como uma categoria ordenada com a sequência desejada
                itens_plano['Refeicao'] = pd.Categorical(itens_plano['Refeicao'], categories=sequencia_refeicao, ordered=True)
                
                # Ordenar o DataFrame com base na coluna 'Refeicao'
                itens_plano = itens_plano.sort_values(by="Refeicao", ascending=True).reset_index(drop=True)

            # Exibindo o editor de dados no Streamlit
...
def render_registro_sub_tab(username: str, user_data: Dict[str, Any]):
    ...
    if 'current_plan_name' not in st.session_state or st.session_state.current_plan_name != todays_plan_name:
        df_to_load = workout_today['exercicios'] if workout_today else pd.DataFrame()
        st.session_state.todays_workout_df = df_to_load.reset_index(drop=True)
    ...
    if workout_today and not st.session_state.todays_workout_df.empty:
        exercicios_df = st.session_state.todays_workout_df
                ...
                with col_main:
                    ...
                    with st.popover(f"**{exercicio['nome_exercicio']}**"):
                        ...
                        if image_paths and len(image_paths) == 2:
                            ...
                            st.info("Visualização do exercício indisponível.")
    ...
    # Lógica do usuário para o expander de Registro Avulso
...
def render_registro_avulso_form(username: str, user_data: Dict[str, Any]):
    ...
    if st.button("Salvar Treino Avulso", type="primary", use_container_width=True):
...
def render_evolucao_tab(user_data: Dict[str, Any]):
    ...
    with st.expander("Adicionar novas medidas", expanded=False):
        with st.form(key="form_adicionar_medida_evolucao"):
            ...
            if st.form_submit_button("Adicionar medida"):
                ...
                # Atualiza dados pessoais **apenas** se o input for maior que zero
    ...
    # Usa a função de lógica para obter os últimos valores válidos (último != 0 do histórico)
    dados_atuais = logic.get_latest_metrics(dados_pessoais, dfe_final)
    ...
    if not dfe_final.empty:
        ...
        # Gera os gráficos da evolução (usa o histórico)
        ...
        # Editor do histórico — ao salvar, atualiza dados pessoais somente quando valores são > 0
        with st.expander("Histórico de medições", expanded=False):
            dfe_editado = st.data_editor(
                dfe_final.sort_values("semana", ascending=False),
                num_rows="dynamic",
                use_container_width=True,
                hide_index=True,
                key="editor_evolucao"
            ...
            if st.button("💾 Salvar Alterações no Histórico", key="salvar_historico_evolucao"):
                ...
                if not dfp.empty:
                    if not df_para_salvar.empty:
                        ...
                        # Atualiza apenas se campo existir e for > 0
                        try:
                            peso_val = ultima_medida.get(config.COL_PESO, dfp.loc[0, config.COL_PESO])
                            if pd.notna(peso_val) and float(peso_val) > 0:
                                dfp.loc[0, config.COL_PESO] = float(peso_val)
                        except Exception:
                            pass

                        try:
                            gc = ultima_medida.get('gordura_corporal', dfp.loc[0, 'gordura_corporal'])
                            if pd.notna(gc) and float(gc) > 0:
                                dfp.loc[0, 'gordura_corporal'] = float(gc)
                        except Exception:
                            pass

                        try:
                            gv = ultima_medida.get('gordura_visceral', dfp.loc[0, 'gordura_visceral'])
                            if pd.notna(gv) and float(gv) > 0:
                                dfp.loc[0, 'gordura_visceral'] = float(gv)
                        except Exception:
                            pass

                        try:
                            mm = ultima_medida.get('musculos_esqueleticos', dfp.loc[0, 'massa_muscular'])
                            if pd.notna(mm) and float(mm) > 0:
                                dfp.loc[0, 'massa_muscular'] = float(mm)
                        except Exception:
                            pass
