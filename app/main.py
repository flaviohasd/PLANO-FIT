...
tab1, tab2, tab3, tab4, tab5 = st.tabs(["👤 Dados pessoais", "🎯 Objetivos & metas", "🍽️ Alimentação", "🏋️‍♀️ Treino", "📈 Evolução"])
...
with tab4:
    st.header("🏋️‍♀️ Treinos e Gasto calórico")
    ...
    with st.form("form_add_treino"):
        data_treino = st.text_input(f"Data do treino (DD/MM/AAAA) — vazio = {datetime.today().strftime('%d/%m/%Y')}", value="")
    ...
    if sbm_treino:
        ...
        novo = pd.DataFrame([
            {"data": data_reg, "tipo treino": tipo_str, "duracao min": duracao_min, "calorias": round(gasto_est, 2)}
        ])
    ...
    if os.path.exists(PATH_TREINOS):
        ...
        if not dft.empty:
            dft["data"] = pd.to_datetime(dft["data"], format="%d/%m/%Y")
            total_treinos = len(dft)
            total_calorias = dft["calorias"].sum()
            dft["semana do ano"] = dft["data"].dt.isocalendar().week.astype(int)
            semanas = dft["semana do ano"].nunique()
            ...
            treinos_semana = dft[dft["semana do ano"] == semana_atual].shape[0]
            ...
            with st.expander("Histórico de Treinos", expanded=True):
